 ...
 */
void Cmd_Wait_f(void) { cmd_wait = Sys_Milliseconds(); }
...
qboolean Cbuf_AddLateCommands(void) {
  ...
  for (i = 0; i < s - 1; i++) {
    if (text[i] == '+') {
      ...
      for (j = i; (text[j] != '+') && !(text[j] == '-' && text[j - 1] == ' ') &&
                  (text[j] != 0);
           j++) {
      }
...
static const char *additional_commands[] = {
    "team",
    ...
    "yes", "no", "mapvote", "votemap", "votetime", "timevote",

    "maplist", "votelist", "mapsleft", "mapsdone", "playerlist", "playertimes",
    "playerscores", "playermaps", "maptimes", "maptimeswp", "1st", "lastmaps",

    "jumpers", "showjumps",

    "store", "recall",

    "showtimes", "velstore", "cpsound", "mute_cprep",
    ...
    // admins stuff
    //
    "addmap", "addball", "addtime", "addbox", "addclip", "skinent", "skinbox",
    "movebox", "moveent", "remallents", "bring", "cvote", "pvote", "mvote",
    "dvotes", "slap", "rand", "nominate", "changepass", "admin", "unadmin",
    "mset", "gset", "aset", "lock", "remtime", "remalltimes", "togglehud",
    "nextmaps", "changename", "addadmin", "remadmin", "changeadmin",
    "listadmin", "ratereset", "sortmaps", "addent", "alignent", "shiftent",
    "listents", "entlist", "rement", "throwup", "remmap", "debug",
    "updatescores", "playtag", "autorecord", "cmsg", "ghost", "boot", "silence",
    "unsilence", "dummyvote", "banflags", "whois",

    // misc
    "autoadmin", // Literally does nothing.
    "players", "say_person", "p_say",
    "!w",      // pm
    "steam",   // same as say_team
    "putaway", // close all menus
    "uptime", "chaseme", "cleanhud", "compare", "flashlight", "id", "!stats",
    "stats", "time", "coord", "drop", "skinlist", NULL};
...
char *Cmd_CompleteCommand(char *partial) {
  ...
  for (o = 0; additional_commands[o]; o++) {
    if (!strcmp(partial, additional_commands[o])) {
      return (char *)additional_commands[o];
    }
  }
  ...
  for (o = 0; additional_commands[o]; o++) {
    if (!strncmp(partial, additional_commands[o], len)) {
      pmatch[i] = (char *)additional_commands[o];
      i++;
    }
  }
    ...
    if (nbMatches == 1) {
      ...
      for (j = 0; j < strlen(pmatch[0]); j++) {
        for (k = 1; k < nbMatches; k++) {
          if (j >= strlen(pmatch[k]) ||
              tolower((unsigned char)pmatch[0][j]) !=
                  tolower((unsigned char)pmatch[k][j])) {
...
void Cmd_ExecuteString(char *text) {
  ...
  if (Cmd_Argc() > 1 && Q_strcasecmp(cmd_argv[0], "exec") == 0 &&
      Q_strcasecmp(cmd_argv[1], "yq2.cfg") == 0) {
