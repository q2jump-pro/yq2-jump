...
static void Qcommon_Buildstring(void) {
  int i;
  int verLen;
  const char *versionString;

  versionString = va("Yamagi Quake II Jump v%s", YQ2JUMPVERSION);
...
static void Qcommon_Mainloop(void) {
  long long newtime;
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    for (int h = 0; h < numHelpArgs; ++h) {
      if (Q_stricmp(arg, helpArgs[h]) == 0) {
        ...
        printf("-datadir <path>\n");
        printf("  set path to your Quake2 game data (the directory baseq2/ is "
               "in)\n");
        printf("-portable\n");
        printf("  Write (savegames, configs, ...) in the binary directory\n");
        printf("+exec <config>\n");
        printf("  execute the given config (mainly relevant for dedicated "
               "servers)\n");
        printf("+set <cvarname> <value>\n");
        printf("  Set the given cvar to the given value, e.g. +set "
               "vid_fullscreen 0\n");
        ...
        printf(
            "  if r_mode is set to -1, these cvars allow you to specify the\n");
        ...
        printf("\nSee "
               "https://github.com/yquake2/yquake2/blob/master/doc/"
               "04_cvarlist.md\nfor some more cvars\n");
...
static void Qcommon_Frame(int usec) {
  ...
  if (R_IsVSyncActive()) {
    ...
    // (possibly not 100% correctly reported) refreshRate would introduce
    // jittering, so only use vid_maxfps if it looks like the user really means
    // it to be different from refreshRate
    if (vid_maxfps->value < refreshrate - 2) {
      rfps = vid_maxfps->value;
      // we can't have more packet frames than render frames, so limit pfps to
      // rfps
      pfps = (cl_maxfps->value > rfps) ? rfps : cl_maxfps->value;
    } else // target refresh rate, not vid_maxfps
    {
      /* if vsync is active, we increase the target framerate a bit for two
         reasons
         1. On Windows, GLimp_GetFrefreshRate() (or the SDL counterpart, or even
            ...
            like 58 or 59 when it's really 59.95 and thus (as integer) should be
         60
      ...
      // we can't have more packet frames than render frames, so limit pfps to
      // rfps but in this case use tolerance for comparison and assign rfps with
      // tolerance
  ...
  // cl_maxfps <= 0 means: automatically choose a packet framerate that should
  // work well with the render framerate, which is the case if rfps is a
  // multiple of pfps
  if (cl_maxfps->value <= 0.0f && cl_async->value != 0.0f) {
    ...
    if (rfps < 90.0f) {
      ...
      /* we want an integer divider, so every div-th renderframe is a
         packetframe. this formula gives nice dividers that keep pfps as close
         as possible to 60 (which seems to be ideal):
  ...
  if (!cl_timedemo->value) {
    if (cl_async->value) {
      ...
      // because we must have at least one render frame between two packet
      // frames
      // TODO: does it make sense to use the average renderdelta of the last X
      // frames
  ...
  if (packetframe || renderframe) {
    CL_Frame(packetdelta, renderdelta, clienttimedelta, packetframe,
             renderframe);
    clienttimedelta = 0;
  }
