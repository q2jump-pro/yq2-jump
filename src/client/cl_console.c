...
void Con_DrawInput(void) {
  ...
  if (linepos >= (con.linewidth - 1)) {
    ...
    Draw_CharScaled(8 * scale, con.vislines - 22 * scale, CON_INPUT_INDICATOR,
                    scale);
    draw_icon = 1;
  }
  ...
  for (i = 0; i < (con.linewidth - draw_icon); i++) {
    ...
    Draw_CharScaled(((i + 1 + draw_icon) << 3) * scale,
                    con.vislines - 22 * scale, ch, scale);
  }
}
...
void Con_DrawNotify(void) {
  ...
  if (cls.key_dest == key_message) {
    ...
    Draw_CharScaled(((x + skip) << 3) * scale, v * scale,
                    10 + ((cls.realtime >> 8) & 1), scale);
    v += 8;
  }
...
void Con_DrawConsole(float frac) {
  ...
  Com_sprintf(version, sizeof(version), "Yamagi Quake II Jump v%s",
              YQ2JUMPVERSION);

  verLen = strlen(version);

  for (x = 0; x < verLen; x++) {
    Draw_CharScaled(viddef.width - ((verLen * 8 + 5) * scale) + x * 8 * scale,
                    lines - 35 * scale, 128 + version[x], scale);
  }
  ...
  for (x = 0; x < 21; x++) {
    Draw_CharScaled(viddef.width - (173 * scale) + x * 8 * scale,
                    lines - 25 * scale, 128 + tmpbuf[x], scale);
  }
