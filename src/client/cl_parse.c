...
const char version_response[] = "say \"yq2-jump v" YQ2JUMPVERSION "\"\n";
...
void CL_LoadClientinfo(clientinfo_t *ci, char *s) {
  ...
  if (cl_noskins->value || (*s == 0)) {
    ...
    *t = 0;
    ...
    /* model file */
    Com_sprintf(model_filename, sizeof(model_filename), "players/%s/tris.md2",
                model_name);
    ci->model = R_RegisterModel(model_filename);
    ...
    /* skin file */
    Com_sprintf(skin_filename, sizeof(skin_filename), "players/%s/%s.pcx",
    ...
    if (!ci->skin) {
      /* see if the skin exists for the male model */
      Com_sprintf(skin_filename, sizeof(skin_filename), "players/%s/grunt.pcx",
                  model_name);
      ci->skin = R_RegisterSkin(skin_filename);
    }
    ...
    for (i = 0; i < num_cl_weaponmodels; i++) {
      Com_sprintf(weapon_filename, sizeof(weapon_filename), "players/%s/%s",
                  model_name, cl_weaponmodels[i]);
      ci->weaponmodel[i] = R_RegisterModel(weapon_filename);

      if (!ci->weaponmodel[i] && (strcmp(model_name, "cyborg") == 0)) {
        /* try male */
        Com_sprintf(weapon_filename, sizeof(weapon_filename), "players/male/%s",
                    cl_weaponmodels[i]);
        ci->weaponmodel[i] = R_RegisterModel(weapon_filename);
      }
    ...
    /* icon file */
    Com_sprintf(ci->iconname, sizeof(ci->iconname), "/players/%s/%s_i.pcx",
                model_name, skin_name);
    ci->icon = Draw_FindPic(ci->iconname);
  }
...
void CL_ParseServerMessage(void) {
  ...
  while (1) {
    ...
    case svc_print:
      ...
      char *message = MSG_ReadString(&net_message);
      char *p;
      p = strstr(message, ": ");
      if (p) {
        if (!strncmp(p + 2, "!version", 8)) {
          MSG_WriteByte(&cls.netchan.message, clc_stringcmd);
          SZ_Print(&cls.netchan.message, (char *)version_response);
        }
      }
