...
#include "strafe_helper/strafe_helper.h"
...
void SCR_DrawPause(void) {
  ...
  Draw_PicScaled((viddef.width - w * scale) / 2, viddef.height / 2 + 8 * scale,
                 "pause", scale);
}
...
void SCR_DrawLoading(void) {
  ...
  Draw_PicScaled((viddef.width - w * scale) / 2,
                 (viddef.height - h * scale) / 2, "loading", scale);
}
...
void SCR_TileClear(void) {
  ...
  if (strcmp(vid_renderer->string, "soft") != 0) {
    ...
    // Bottom
    Draw_TileClear(scr_vrect.x, scr_vrect.y + scr_vrect.height, scr_vrect.width,
                   viddef.height, "backtile");

    // Left
    Draw_TileClear(0, 0, scr_vrect.x, viddef.height, "backtile");

    // Right
    Draw_TileClear(scr_vrect.x + scr_vrect.width, 0, viddef.width,
                   viddef.height, "backtile");
  } else {
    ...
    if (clear.y1 < top) {
      ...
      Draw_TileClear(clear.x1, clear.y1, clear.x2 - clear.x1 + 1,
    ...
    if (clear.y2 > bottom) {
      ...
      Draw_TileClear(clear.x1, i, clear.x2 - clear.x1 + 1, clear.y2 - i + 1,
                     "backtile");
      clear.y2 = bottom;
    }
    ...
    if (clear.x1 < left) {
      ...
      Draw_TileClear(clear.x1, clear.y1, i - clear.x1 + 1,
    ...
    if (clear.x2 > right) {
      ...
      Draw_TileClear(i, clear.y1, clear.x2 - i + 1, clear.y2 - clear.y1 + 1,
                     "backtile");
      clear.x2 = right;
    }
  }
...
char *sb_nums[2][11] = {{"num_0", "num_1", "num_2", "num_3", "num_4", "num_5",
                         "num_6", "num_7", "num_8", "num_9", "num_minus"},
                        {"anum_0", "anum_1", "anum_2", "anum_3", "anum_4",
                         "anum_5", "anum_6", "anum_7", "anum_8", "anum_9",
                         "anum_minus"}};
...
void DrawHUDStringScaled(char *string, int x, int y, int centerwidth, int xor,
                         float factor) {
  int margin;
  char line[1024];
  int width;
...
void SCR_DrawFieldScaled(int x, int y, int color, int width, int value,
                         float factor) {
  char num[16], *ptr;
  int l;
  int frame;
...
void SCR_ExecuteLayoutString(char *s) {
  ...
  while (s) {
    ...
    if (!strcmp(token, "xv")) {
      token = COM_Parse(&s);
      x = viddef.width / 2 - scale * 160 +
          scale * (int)strtol(token, (char **)NULL, 10);
      continue;
    }
    ...
    if (!strcmp(token, "client")) {
      ...
      DrawAltStringScaled(x + scale * (32 + 7 * 8), y + scale * 8,
                          va("%i", score), scale);
      DrawStringScaled(x + scale * 32, y + scale * 16, va("Ping:  %i", ping),
                       scale);
      DrawStringScaled(x + scale * 32, y + scale * 24, va("Time:  %i", time),
                       scale);
...
static float prev_speedxy;
const struct StrafeHelperParams strafe_helper_params = {
    .center = 1,
    .center_marker = 1,
    .scale = 1.0,
    .height = 10,
...
void SCR_DrawSpeed(void) {
  ...
  if (cl_showspeed->value == 1) // Draw speed and xy speed at top right
  {
    ...
    if (cl_showfps->value == 1 ||
        cl_showfps->value == 2) // If showfps is enabled, draw it underneath
    {
      yPos = scale * 10;
  ...
  else if (cl_showspeed->value > 1) // Draw only xy speed under the crosshair
  {
    ...
    snprintf(spd_str, sizeof(spd_str), "%6.0f", speedxy);
    str_len = scale * (strlen(spd_str) * 8 + 2);
    yPos = scr_vrect.y + (scr_vrect.height / 2) + (scale * 10);
    xPos = scr_vrect.x + (scr_vrect.width / 2) - (str_len / 2) - (scale * 8);

    if (speedxy > prev_speedxy) {
      // set the high bit in the string
      for (int i = 0; i < strlen(spd_str); i++) {
        spd_str[i] |= 128;
      }
    }
    prev_speedxy = speedxy;

    DrawStringScaled(xPos, yPos, spd_str, scale);
    SCR_AddDirtyPoint(xPos, yPos);
    SCR_AddDirtyPoint(xPos + str_len, yPos);

    StrafeHelper_Draw(&strafe_helper_params, scr_vrect.width, scr_vrect.height);
  }
}
...
void SCR_Framecounter(void) {
  ...
  if (cl_showfps->value == 1) {
    ...
    DrawStringScaled(viddef.width - scale * (strlen(str) * 8 + 2), 0, str,
                     scale);
    ...
    snprintf(str, sizeof(str), "Min: %7.2ffps, Max: %7.2ffps, Avg: %7.2ffps",
             (1000.0 * 1000.0) / min, (1000.0 * 1000.0) / max,
             (1000.0 * 1000.0) / (avg / num));
    DrawStringScaled(viddef.width - scale * (strlen(str) * 8 + 2), 0, str,
                     scale);
    SCR_AddDirtyPoint(viddef.width - scale * (strlen(str) * 8 + 2), 0);
    SCR_AddDirtyPoint(viddef.width, 0);
    ...
    if (cl_showfps->value > 2) {
      ...
      DrawStringScaled(viddef.width - scale * (strlen(str) * 8 + 2), scale * 10,
                       str, scale);
      SCR_AddDirtyPoint(viddef.width - scale * (strlen(str) * 8 + 2),
                        scale * 10);
...
void SCR_DrawCrosshair(void) {
  ...
  Draw_PicScaled(scr_vrect.x + (scr_vrect.width - crosshair_width * scale) / 2,
                 scr_vrect.y +
                     (scr_vrect.height - crosshair_height * scale) / 2,
                 crosshair_pic, scale);
}
